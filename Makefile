...
CC = gcc
GCCVERSION_480 := $(shell expr `gcc -dumpversion | cut -d. -f1` \>= 4)
GCCVERSION_700 := $(shell expr `gcc -dumpversion | cut -d. -f1` \>= 7)
PYVERSION_270 := $(shell expr `python3 --version 2>&1 |cut -c 8- | sed -e 's/\.\([0-9][0-9]\)/\1/g' -e 's/\.\([0-9]\)/0\1/g' -e 's/^[0-9]\{3,4\}$$/&00/'` \>= 30000)
